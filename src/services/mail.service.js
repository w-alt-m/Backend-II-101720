...
export const sendCancellationEmail = async ({ to, userName, eventTitle, eventDate, eventLocation, quantity, reservationCode }) => {
  const formattedDate = new Date(eventDate).toLocaleDateString("es-AR", {
    weekday: "long",
    year: "numeric",
    month: "long",
  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>Hola <strong>${userName}</strong>,</p>
        <p>Tu inscripción al siguiente evento fue cancelada exitosamente.</p>
        <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
          <tr>
            <td style="padding: 8px; border: 1px solid #ddd; font-weight: bold;">Evento</td>
            <td style="padding: 8px; border: 1px solid #ddd;">${eventTitle}</td>
        ...
        <p>El cupo fue liberado y está disponible para otros usuarios.</p>
      </div>
    `
  };
